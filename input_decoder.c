...
int main (int argc, char **argv) {
	...
	int abs[5];
	char date_out[200]; //fecha impresa
	char date_format[200] = "%a, %d %b %Y %T %z"; //formato impresion fecha

	time_t nowtime;
	struct tm *nowtm;
	char tmbuf[64], date_buf[64];

	if (argc < 2) {
		printf("USAGE: ./input_decoder /dev/input/eventX\n");
		return 1;
	}
...
/*
 ...
 28         __u16 type; 2 bytes
 29         __u16 code; 2 bytes
 30         __s32 value; 4 bytes
 31 };
 32  
 ...
 */
	...
	while (1) {
		...
		if (rd < (int) sizeof(struct input_event)) {
			perror("input_event struct seems not well formated");
			return 1;
		}
//http://www.kernel.org/doc/Documentation/input/event-codes.txt
		...
		//	printf("%0x  \n", ((unsigned int *) ev)[i]);
			...
			printf("\ntime:%x type:%x code:%x value:%x \n", ev[i].time, ev[i].type, ev[i].code, ev[i].value);
			
			gettimeofday(&(ev[i].time), NULL);
			nowtime = ev[i].time.tv_sec;
			nowtm = localtime(&nowtime);
			strftime(tmbuf, sizeof tmbuf, "%Y-%m-%d %H:%M:%S", nowtm);
			snprintf(date_buf, sizeof date_buf, "%s.%06ld", tmbuf, ev[i].time.tv_usec);

			if (ev[i].type == EV_SYN) {
				printf("Event: time %s unix:%ld.%06ld type:%d (Sync) code:%d (%s)\n", date_buf,
					ev[i].time.tv_sec, ev[i].time.tv_usec,  ev[i].type, ev[i].code, syn[ev[i].code]);
				
			} else if (ev[i].type == EV_MSC && (ev[i].code == MSC_RAW || ev[i].code == MSC_SCAN)) {
				printf("Event: time %s unix:%ld.%06ld type:%d (%s) code:%d (%s) value:%02x\n", date_buf,
				...
				printf("Event: time %s unix:%ld.%06ld type:%d (%s) code:%d (%s) value:%d (%s)\n", date_buf,
					ev[i].time.tv_sec, ev[i].time.tv_usec, ev[i].type,
					events[ev[i].type] ? events[ev[i].type] : "?",
					ev[i].code,
					names[ev[i].type] ? (names[ev[i].type][ev[i].code] ? names[ev[i].type][ev[i].code] : "?") : "?",
					ev[i].value, 
					(ev[i].type == EV_KEY)?key_values[ev[i].value]:"?"
  					);
			}	
		}
	}
